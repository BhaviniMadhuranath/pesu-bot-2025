    ...
    async def on_message(self, message):
        if(message.author.bot):
            ...
            if (f'<@!{BOT_DEVS}>' in str(temp)):  # Bot devs
                ...
                ping_embed.add_field(name="Ping report",
                                     value=ping_log, inline=False)
                ping_embed.add_field(
                    name="Message content", value=f"https://discord.com/channels/{GUILD_ID}/{message.channel.id}/{message.id}", inline=False)
    ...
    async def _help(self, ctx):
        ...
        if((self.admin in ctx.author.roles) or (self.mods in ctx.author.roles) or (self.bot_devs in ctx.author.roles)):
            ...
            if((self.admin in ctx.author.roles) or (self.mods in ctx.author.roles)):
                help_embed.add_field(name="Enable Confess",
                                     value=self.confessen)
                help_embed.add_field(
                    name="Disable Confess", value=self.confessdis)
